...
const YieldFarmingModule = {
    ...
    showFarmModal(farmId) {
        ...
        // Map farm risk text to risk level number
        ...
        if (typeof InvestHelper !== 'undefined') {
            InvestHelper.show({
                ...
                minInvest: 10,
                fee: 0,
                risk: riskMap[farm.risk] || 3,
                icon: farm.icon || '🌾',
                onInvest: (amount, mode) => {
                    this.executeFarm(farm, amount, mode);
                }
            });
        } else {
            this.showSimpleFarmModal(farmId);
        }
    },

    executeFarm(farm, amount, mode) {
        if (mode === 'simulated') {
            const result = this.deposit(farm.id, amount);
            if (!result.success && typeof showNotification === 'function') {
                showNotification(result.error, 'error');
            }
        } else {
            // Real mode - would integrate with actual farming protocols
            if (typeof WalletManager !== 'undefined' && WalletManager.isConnected && WalletManager.isConnected()) {
                console.log('[FARMING] Real farm:', amount, farm.name);
                if (typeof showNotification === 'function') {
                    showNotification('Real farming started for $' + amount + ' in ' + farm.name, 'success');
                }
            }
        }
    },

    showSimpleFarmModal(farmId) {
        const farm = this.farms.find(f => f.id === farmId);
        if (!farm) return;
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;max-width:400px;width:90%;">
                ...
                <button onclick="YieldFarmingModule.confirmFarm('${farm.id}')" style="width:100%;padding:12px;background:#00ff88;border:none;border-radius:8px;color:#000;font-weight:bold;cursor:pointer;">Start Farming</button>
            </div>
        `;
