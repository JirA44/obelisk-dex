...
const WalletConnect = {
    ...
    async getBalance() {
        ...
        try {
            ...
            // Also fetch USDC balance and update portfolio
            await this.fetchAndUpdateUSDCBalance();
        } catch (e) {
            console.error('[WalletConnect] Balance error:', e);
        }
    },

    // USDC contract addresses by chain (native USDC)
    USDC_CONTRACTS: {
    ...
    // USDC.e (bridged) contract addresses - some chains have both
    USDC_BRIDGED_CONTRACTS: {
        ...
        10: '0x7F5c764cBc14f9669B88837ca1490cCa17c31607',     // Optimism USDC.e (bridged)
        43114: '0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664', // Avalanche USDC.e
    },

    // Fetch USDC balance from blockchain (checks both native and bridged USDC)
    async getUSDCBalance() {
        if (!this.address || !this.provider || !this.chainId) {
            console.log('[WalletConnect] Cannot fetch USDC: missing address, provider or chainId');
            return 0;
        }

        let totalBalance = 0;

        // Helper to fetch balance for a specific token address
        const fetchTokenBalance = async (tokenAddress, tokenName) => {
            try {
                ...
                if (balance > 0) {
                    console.log(`[WalletConnect] ${tokenName} Balance: $${balance.toFixed(2)}`);
                }
                return balance;
            } catch (e) {
                console.error(`[WalletConnect] ${tokenName} balance error:`, e);
                return 0;
            }
        };

        // Check native USDC
        const nativeUsdcAddress = this.USDC_CONTRACTS[this.chainId];
        if (nativeUsdcAddress) {
            totalBalance += await fetchTokenBalance(nativeUsdcAddress, 'USDC Native');
        }

        // Check bridged USDC.e
        const bridgedUsdcAddress = this.USDC_BRIDGED_CONTRACTS[this.chainId];
        if (bridgedUsdcAddress) {
            totalBalance += await fetchTokenBalance(bridgedUsdcAddress, 'USDC.e Bridged');
        }

        if (totalBalance === 0 && !nativeUsdcAddress && !bridgedUsdcAddress) {
            console.log('[WalletConnect] USDC not supported on chain', this.chainId);
        }

        console.log(`[WalletConnect] Total USDC Balance: $${totalBalance.toFixed(2)} on chain ${this.chainId}`);

        // Store balance for other modules to access
        this.usdcBalance = totalBalance;

        return totalBalance;
    },

    // Fetch USDC and update SimulatedPortfolio real balance
    async fetchAndUpdateUSDCBalance() {
        const usdcBalance = await this.getUSDCBalance();
        const isFr = (typeof I18n !== 'undefined' && I18n.currentLang === 'fr');

        if (typeof SimulatedPortfolio !== 'undefined') {
            // Get current balance
            const currentReal = SimulatedPortfolio.portfolio.realBalance || 0;

            // Update if balance changed
            if (Math.abs(usdcBalance - currentReal) > 0.001) {
                // Set the real balance directly
                SimulatedPortfolio.portfolio.realBalance = usdcBalance;
                SimulatedPortfolio.saveState();
                SimulatedPortfolio.updateUI();

                console.log(`[WalletConnect] Portfolio real balance updated: $${usdcBalance.toFixed(2)}`);

                // Show notification
                if (usdcBalance > 0) {
                    this.showNotification(
                        isFr ? `💎 Solde USDC détecté: $${usdcBalance.toFixed(2)}` : `💎 USDC balance detected: $${usdcBalance.toFixed(2)}`,
                        'success'
                    );
                } else {
                    this.showNotification(
                        isFr ? `💎 Aucun USDC détecté dans le wallet (chain ${this.chainId})` : `💎 No USDC found in wallet (chain ${this.chainId})`,
                        'info'
                    );
                }
            }
        ...
        window.dispatchEvent(new CustomEvent('usdc-balance-updated', {
            detail: { balance: usdcBalance, address: this.address, chainId: this.chainId }
        }));
