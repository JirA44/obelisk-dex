...
const ToolsUI = {
    ...
    cancelIntent(intentId) {
        if (SmartIntent.cancelIntent(intentId)) {
            ...
            this.showNotification(isFr ? 'Intention annulée' : 'Intent cancelled', 'success');
            this.updateIntentsList();
        }
    },
    ...
    initArbitrageScanner() {
        ...
        if (startBtn) {
            startBtn.addEventListener('click', () => {
                ...
                this.showNotification(isFr1 ? 'Scanner d\'arbitrage démarré' : 'Arbitrage scanner started', 'success');
            });
        }
        ...
        if (stopBtn) {
            stopBtn.addEventListener('click', () => {
                ...
                this.showNotification(isFr2 ? 'Scanner arrêté' : 'Scanner stopped', 'info');
            });
        }
        ...
        if (setupForm) {
            setupForm.addEventListener('submit', async (e) => {
                ...
                if (!wallet) {
                    ...
                    this.showNotification(isFr3 ? 'Connectez d\'abord votre wallet' : 'Connect wallet first', 'error');
                    return;
                }
                ...
                if (addresses.length < 2) {
                    const isFr4 = typeof I18n !== 'undefined' && I18n.currentLang === 'fr';
                    this.showNotification(isFr4 ? 'Il faut au moins 2 gardiens' : 'Need at least 2 guardians', 'error');
                    return;
                }

                try {
                    const result = await SocialRecovery.setupGuardians(wallet, addresses, threshold, password);
                    const isFr5 = typeof I18n !== 'undefined' && I18n.currentLang === 'fr';
                    this.showNotification(isFr5 ? `${result.guardians.length} gardiens configurés` : `${result.guardians.length} guardians configured`, 'success');
    ...
    initAIAdvisor() {
        ...
        if (analyzeBtn) {
            analyzeBtn.addEventListener('click', async () => {
                const wallet = WalletManager.currentWallet;
                const isFr6 = typeof I18n !== 'undefined' && I18n.currentLang === 'fr';
                if (!wallet) {
                    this.showNotification(isFr6 ? 'Connectez d\'abord votre wallet' : 'Connect wallet first', 'error');
                    return;
                }

                analyzeBtn.disabled = true;
                analyzeBtn.textContent = isFr6 ? 'Analyse en cours...' : 'Analyzing...';
                ...
                try {
                    ...
                    analyzeBtn.textContent = isFr6 ? 'Analyser Portfolio' : 'Analyze Portfolio';
                }
            });
        }
    },
