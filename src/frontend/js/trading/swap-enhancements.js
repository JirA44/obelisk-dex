...
const SwapEnhancements = {
    ...
    enhanceSwapButton() {
        ...
        // Verifier etat de connexion et mettre a jour le bouton
        this.updateSwapButtonState();

        // Observer les changements d'auth
        if (typeof FirebaseAuth !== 'undefined') {
            FirebaseAuth.onAuthChange(() => this.updateSwapButtonState());
        }
        ...
        btnSwap.onclick = async (e) => {
            ...
            if (!connectionState.isConnected) {
                // Pas connecte du tout - ouvrir le modal auth
                if (typeof AuthUI !== 'undefined') {
                    AuthUI.show('login');
                } else if (typeof WalletConnect !== 'undefined' && WalletConnect.openModal) {
                    WalletConnect.openModal();
                }
                return;
            }

            // Connecte - executer le swap
            if (connectionState.mode === 'simulated') {
                this.executeSimulatedSwap();
    ...
    getConnectionState() {
        // Verifier Firebase (Google, email, etc.)
        const isFirebaseConnected = typeof FirebaseAuth !== 'undefined' && FirebaseAuth.isSignedIn && FirebaseAuth.isSignedIn();

        // Verifier wallet crypto
        ...
        // Verifier portfolio simule
        const hasSimulatedBalance = typeof SimulatedPortfolio !== 'undefined' &&
                                    SimulatedPortfolio.getBalance &&
                                    SimulatedPortfolio.getBalance() > 0;

        return {
            isConnected: isFirebaseConnected || isWalletConnected,
            isFirebase: isFirebaseConnected,
            isWallet: isWalletConnected,
            hasSimulated: hasSimulatedBalance,
            mode: isWalletConnected ? 'wallet' : (isFirebaseConnected ? 'simulated' : null)
        };
    },
    ...
    updateSwapButtonState() {
        ...
        if (state.isConnected) {
            ...
            if (state.mode === 'simulated') {
                btnSwap.innerHTML = '🎮 Swap (Simulé)';
                btnSwap.style.background = 'linear-gradient(135deg, #a855f7, #7c3aed)';
            } else {
            ...
            btnSwap.textContent = 'Connectez-vous pour échanger';
            btnSwap.style.background = '';
        }

        // Mettre a jour les soldes affiches
        this.updateBalanceDisplay(state);
    },

    updateBalanceDisplay(state) {
        // Mettre a jour le solde "From" token
        const fromBalance = document.querySelector('.swap-input-group .token-balance, .from-balance');
        if (fromBalance && state.mode === 'simulated' && typeof SimulatedPortfolio !== 'undefined') {
            const balance = SimulatedPortfolio.getBalance();
            fromBalance.textContent = `Solde: $${balance.toFixed(2)}`;
        }
    },

    executeSimulatedSwap() {
        // Recuperer les valeurs du swap
        const fromAmount = parseFloat(document.getElementById('swap-input')?.value) || 0;
        const fromToken = document.querySelector('.swap-from .token-symbol')?.textContent || 'USDC';
        const toToken = document.querySelector('.swap-to .token-symbol')?.textContent || 'ETH';

        if (fromAmount <= 0) {
            if (typeof showNotification === 'function') {
                showNotification('Entrez un montant valide', 'warning');
            }
            return;
        }

        // Verifier le solde simule
        if (typeof SimulatedPortfolio !== 'undefined') {
            const balance = SimulatedPortfolio.getBalance();
            if (fromAmount > balance) {
                if (typeof showNotification === 'function') {
                    showNotification('Solde simulé insuffisant', 'error');
                }
                return;
            }

            // Simuler le swap (deduire du solde et ajouter la position)
            SimulatedPortfolio.withdraw(fromAmount, `Swap ${fromToken} → ${toToken}`);

            // Calculer le montant recu (simulation avec prix fictif)
            const prices = { ETH: 3200, BTC: 95000, SOL: 180, ARB: 1.2, OP: 2.5 };
            const toPrice = prices[toToken] || 1;
            const toAmount = fromAmount / toPrice;

            if (typeof showNotification === 'function') {
                showNotification(`🎮 Swap simulé: ${fromAmount} ${fromToken} → ${toAmount.toFixed(6)} ${toToken}`, 'success');
            }
            ...
            this.updateSwapButtonState();
            if (SimulatedPortfolio.renderSidebar) SimulatedPortfolio.renderSidebar();
        }
    },
