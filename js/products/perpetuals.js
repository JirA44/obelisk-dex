...
const PerpetualsModule = {
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;max-width:400px;width:90%;">
                ...
                <div style="display:flex;gap:12px;">
                    <button onclick="document.getElementById('perp-modal').remove()" style="flex:1;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#fff;cursor:pointer;">Cancel</button>
                    <button onclick="PerpetualsModule.confirmTrade('${market.id}','${side}')" style="flex:1;padding:12px;background:${sideColor};border:none;border-radius:8px;color:#fff;font-weight:bold;cursor:pointer;">${side.toUpperCase()}</button>
                </div>
            </div>
        `;
    ...
    confirmTrade(marketId, side) {
        ...
        if (!size || size < 10) {
            alert('Minimum size is $10');
            return;
        }

        const result = this.openPosition(marketId, side, size, leverage);
        document.getElementById('perp-modal').remove();

        if (result.success) {
            const market = this.markets.find(m => m.id === marketId);
            if (typeof showNotification === 'function') {
                showNotification('Opened ' + side.toUpperCase() + ' ' + market.symbol + ' $' + size + ' @ ' + leverage + 'x', 'success');
            } else {
