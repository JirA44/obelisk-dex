...
const PortfolioFix = {
    ...
    updateAllBalanceDisplays() {
        ...
        if (typeof SimulatedPortfolio !== 'undefined' && SimulatedPortfolio.portfolio) {
            ...
            balance = safe(p.simulatedBalance);
            realBalance = safe(p.realBalance);
            // Calculer invested/earnings depuis les investments
            const invests = p.investments || [];
            invested = invests.reduce((sum, inv) => sum + safe(inv.amount), 0);
            earnings = invests.reduce((sum, inv) => sum + safe(inv.earnings), 0);
            productsCount = invests.length;
        } else {
            ...
            try {
                ...
                if (saved) {
                    ...
                    balance = safe(portfolio.simulatedBalance);
                    realBalance = safe(portfolio.realBalance);
                    const invests = portfolio.investments || [];
                    invested = invests.reduce((sum, inv) => sum + safe(inv.amount), 0);
                    earnings = invests.reduce((sum, inv) => sum + safe(inv.earnings), 0);
                    productsCount = invests.length;
                }
        ...
        const globalTotal = balance + realBalance + invested + earnings;
        ...
        const realBalanceEl = document.getElementById('portfolio-real-balance');
        ...
        const globalTotalEl = document.getElementById('portfolio-global-total');
        ...
        if (realBalanceEl) realBalanceEl.textContent = '$' + this.formatAmount(realBalance);
        if (totalInvestedEl) totalInvestedEl.textContent = '$' + this.formatAmount(invested);
        if (totalEarningsEl) totalEarningsEl.textContent = (earnings >= 0 ? '+' : '') + '$' + this.formatAmount(Math.abs(earnings));
        if (globalTotalEl) globalTotalEl.textContent = '$' + this.formatAmount(globalTotal);
        if (productsCountEl) productsCountEl.textContent = productsCount.toString();
    },

    /**
     * Add real balance - requires Web3 wallet connection
     * Real deposits go through on-chain transactions
     */
    async addReal(amount) {
        // Check if wallet is connected
        if (typeof WalletConnect === 'undefined' || !WalletConnect.isConnected()) {
            if (typeof showNotification === 'function') {
                showNotification('Connectez votre wallet pour un dépôt réel', 'error');
            }
        ...
        try {
            // Route through ObeliskLive deposit if available
            if (typeof ObeliskLive !== 'undefined') {
                ObeliskLive.showDepositModal();
                return true;
            }

            // Fallback: update real balance locally
            if (typeof SimulatedPortfolio !== 'undefined' && SimulatedPortfolio.portfolio) {
            ...
            this.updateAllBalanceDisplays();
            if (typeof showNotification === 'function') {
                showNotification(`+$${amount.toLocaleString()} dépôt réel effectué`, 'success');
            }
            return true;
        } catch (e) {
            console.error('[PortfolioFix] Real deposit failed:', e);
            if (typeof showNotification === 'function') {
                showNotification('Erreur de dépôt: ' + e.message, 'error');
            }
            return false;
        }
    },
