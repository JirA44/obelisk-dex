...
const OptionsModule = {
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;max-width:400px;width:90%;">
                ...
                <div style="background:linear-gradient(135deg,rgba(0,212,170,0.1),rgba(0,212,170,0.05));border:1px solid rgba(0,212,170,0.3);border-radius:10px;padding:12px;margin-bottom:10px;">
                    <div style="color:#00d4aa;font-size:11px;font-weight:600;text-align:center;margin-bottom:8px;">📥 ${typeof I18n !== 'undefined' && I18n.currentLang === 'fr' ? 'Acheter Option' : 'Buy Option'}</div>
                    <div style="display:flex;gap:8px;">
                        <button onclick="OptionsModule.confirmBuy('${opt.id}',true)" style="flex:1;padding:10px;border-radius:6px;border:none;background:linear-gradient(135deg,#a855f7,#7c3aed);color:#fff;font-size:12px;font-weight:600;cursor:pointer;">🎮 ${typeof I18n !== 'undefined' && I18n.currentLang === 'fr' ? 'SIMULÉ' : 'SIMULATED'}</button>
                        <button onclick="OptionsModule.confirmBuy('${opt.id}',false)" style="flex:1;padding:10px;border-radius:6px;border:none;background:linear-gradient(135deg,#00d4aa,#00a884);color:#fff;font-size:12px;font-weight:600;cursor:pointer;">💎 ${typeof I18n !== 'undefined' && I18n.currentLang === 'fr' ? 'RÉEL' : 'REAL'}</button>
                    </div>
                </div>
                <button onclick="document.getElementById('options-modal').remove()" style="width:100%;padding:10px;background:transparent;border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#aaa;cursor:pointer;">${typeof I18n !== 'undefined' && I18n.currentLang === 'fr' ? 'Annuler' : 'Cancel'}</button>
            </div>
        `;
    ...
    confirmBuy(optionId, isSimulated = true) {
        ...
        if (!contracts || contracts < 0.01) {
            alert(isFr ? 'Minimum 0.01 contrats' : 'Minimum 0.01 contracts');
            return;
        }
        ...
        const result = this.buy(optionId, contracts, isSimulated);
        ...
        if (result.success) {
            ...
            if (typeof showNotification === 'function') {
                showNotification(modeText + ' ' + (isFr ? 'Acheté ' : 'Bought ') + contracts + ' ' + opt.name + ' - $' + result.cost.toFixed(2), 'success');
            }
        } else {
            alert(result.error);
        }
