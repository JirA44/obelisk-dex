...
const LendingModule = {
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;max-width:400px;width:90%;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                        <span style="color:#888;">Collateral Factor:</span>
                        <span>${market.ltv}%</span>
                    </div>
                </div>
                <div style="margin-bottom:16px;">
                    <label style="display:block;color:#888;margin-bottom:6px;font-size:12px;">Amount USD (min $10)</label>
                    <input type="number" id="supply-amount" min="10" placeholder="Enter amount..." style="width:100%;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#fff;font-size:14px;box-sizing:border-box;">
                </div>
                <div style="display:flex;gap:12px;">
                    <button onclick="document.getElementById('lending-modal').remove()" style="flex:1;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#fff;cursor:pointer;">Cancel</button>
                    <button onclick="LendingModule.confirmSupply('${market.id}')" style="flex:1;padding:12px;background:#00ff88;border:none;border-radius:8px;color:#000;font-weight:bold;cursor:pointer;">Supply</button>
                </div>
            </div>
        `;
    ...
    confirmSupply(marketId) {
        const amount = parseFloat(document.getElementById('supply-amount').value);
        if (!amount || amount < 10) {
            alert('Minimum supply is $10');
            return;
        }

        const result = this.supply(marketId, amount);
        document.getElementById('lending-modal').remove();

        if (result.success) {
            const market = this.markets.find(m => m.id === marketId);
            if (typeof showNotification === 'function') {
                showNotification('Supplied $' + amount + ' ' + market.asset, 'success');
            } else {
                alert('Supplied $' + amount + ' ' + market.asset);
            }
        } else {
            alert(result.error || 'Supply failed');
        }
    },
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;max-width:400px;width:90%;">
                ...
                <div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:8px;margin-bottom:16px;font-size:11px;color:#ef4444;">
                    ⚠️ Borrowing requires collateral. If LTV exceeds ${market.ltv}%, your position may be liquidated.
                </div>
                <div style="display:flex;gap:12px;">
                    <button onclick="document.getElementById('lending-modal').remove()" style="flex:1;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#fff;cursor:pointer;">Cancel</button>
                    <button onclick="LendingModule.confirmBorrow('${market.id}')" style="flex:1;padding:12px;background:#f59e0b;border:none;border-radius:8px;color:#000;font-weight:bold;cursor:pointer;">Borrow</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    confirmBorrow(marketId) {
        ...
        if (!collateral || collateral < 10) {
            alert('Minimum collateral is $10');
            return;
        }

        const maxBorrow = collateral * (market.ltv / 100);
        if (!amount || amount > maxBorrow) {
            alert('Maximum borrow is $' + maxBorrow.toFixed(2));
            return;
        }

        const result = this.borrow(marketId, amount, collateral);
        document.getElementById('lending-modal').remove();

        if (result.success) {
            if (typeof showNotification === 'function') {
                showNotification('Borrowed $' + amount + ' ' + market.asset, 'success');
            } else {
                alert('Borrowed $' + amount + ' ' + market.asset);
            }
        } else {
