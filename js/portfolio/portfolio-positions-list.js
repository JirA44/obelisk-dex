        ...
        if (investments.length === 0) {
            positionsListEl.innerHTML = `
                <div class="positions-empty" style="text-align:center;color:#888;padding:20px;">
                    ...
                    <p style="font-size:12px;margin-top:8px;">Investissez pour voir vos positions ici</p>
                </div>
            `;
        } else {
                    ...
                    ">
                        <div class="position-info" style="flex:1;min-width:150px;">
                            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap;">
                                ...
                                <span style="font-size:10px;background:${bgColor};color:${borderColor};padding:2px 6px;border-radius:4px;">${typeLabel} ${inv.isSimulated ? 'SIM' : 'REEL'}</span>
                            </div>
                            <div style="font-size:11px;color:#888;">
                                ${inv.apy}% APY | ${daysActive} jours | Frais: $${(inv.conversionFee || 0).toFixed(2)}
                            </div>
                        </div>
