...
const AuthUI = {
            ...
            <div class="auth-modal-content">
                ...
                <div class="auth-tabs">
                    <button class="auth-tab ${this.currentMode === 'login' ? 'active' : ''}"
                            onclick="AuthUI.switchMode('login')">Sign In</button>
                    <button class="auth-tab ${this.currentMode === 'register' ? 'active' : ''}"
                            onclick="AuthUI.switchMode('register')">Create Account</button>
                </div>
        ...
        if (this.currentMode === 'login') {
            ...
            return `
                <form class="auth-form" onsubmit="AuthUI.handleForgot(event)">
                    <p class="auth-forgot-text">Enter your email and we'll send you a reset link.</p>
                    <div class="auth-field">
                        <label>Email</label>
                        <input type="email" id="auth-email" placeholder="you@example.com" required>
                    </div>
    ...
    switchMode(mode) {
        ...
        document.querySelectorAll('.auth-tab').forEach(tab => {
            ...
            if (tab.textContent.includes('Sign In') && mode === 'login') tab.classList.add('active');
            if (tab.textContent.includes('Create') && mode === 'register') tab.classList.add('active');
        });
    },
    ...
    async signInGoogle() {
        ...
        if (result.success) {
            this.hide();
            this.showNotification(`Welcome${result.isNewUser ? '' : ' back'}!`, 'success');
    ...
    async connectWallet() {
        ...
        if (typeof connectWallet === 'function') {
            ...
            this.showError('Wallet connection not available');
        }
    },
