...
const LeveragedTokensModule = {
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;max-width:400px;width:90%;">
                ...
                <div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:8px;margin-bottom:16px;font-size:11px;color:#ef4444;">
                    ⚠️ Leveraged tokens can lose value rapidly due to volatility decay
                </div>
                <div style="margin-bottom:16px;">
                    <label style="display:block;color:#888;margin-bottom:6px;font-size:12px;">Amount USD (min $10)</label>
                    <input type="number" id="levtoken-amount" min="10" placeholder="Enter amount..." style="width:100%;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#fff;font-size:14px;box-sizing:border-box;">
                </div>
                <div style="display:flex;gap:12px;">
                    <button onclick="document.getElementById('levtoken-modal').remove()" style="flex:1;padding:12px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:#fff;cursor:pointer;">Cancel</button>
                    <button onclick="LeveragedTokensModule.confirmBuy('${token.id}')" style="flex:1;padding:12px;background:${dirColor};border:none;border-radius:8px;color:#fff;font-weight:bold;cursor:pointer;">Buy Token</button>
                </div>
            </div>
        `;
    ...
    confirmBuy(tokenId) {
        ...
        const result = this.buy(tokenId, amount);
        document.getElementById('levtoken-modal').remove();

        if (result.success) {
            const token = this.tokens.find(t => t.id === tokenId);
            if (typeof showNotification === 'function') {
                showNotification('Bought $' + amount + ' of ' + token.name, 'success');
            } else {
                alert('Bought ' + token.name);
            }
        } else {
            alert(result.error);
        }
