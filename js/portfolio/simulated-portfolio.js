...
const SimulatedPortfolio = {
    ...
    getPnLByPeriod() {
        ...
        this.portfolio.investments.forEach(inv => {
            ...
            daily += inv.amount * dailyRate;
            weekly += inv.amount * dailyRate * 7;
            monthly += inv.amount * dailyRate * 30;
            yearly += inv.amount * (inv.apy / 100);
        });
     ...
     */
    calculateInvPnL(inv) {
        const days = (Date.now() - new Date(inv.timestamp || inv.startDate).getTime()) / (1000 * 60 * 60 * 24);
        return inv.amount * (inv.apy / 100) * (days / 365);
    },
    ...
    getTotalValue() {
        ...
        this.portfolio.investments.forEach(inv => {
            ...
            totalEarnings += inv.earnings;
            if (inv.isSimulated) {
                simulatedInvested += inv.amount;
            } else {
                realInvested += inv.amount;
            }
        });

        const simBalance = this.portfolio.simulatedBalance || 0;
        const realBalance = this.portfolio.realBalance || 0;
                ...
                <div class="portfolio-body">
                    ...
                    <div class="portfolio-summary dual">
                        ...
                        <div class="summary-card real-balance">
                            <span class="summary-label">💎 Solde REEL</span>
                        ...
                        <div class="summary-card earnings">
                            ...
                            <span class="summary-value positive" id="modal-earnings">+$${totals.earnings.toFixed(2)}</span>
                        </div>
            ...
            <div class="investment-product-group">
                ...
                <div class="product-stats">
                    <div class="stat-row simulated">
                        ...
                        <span class="stat-earnings positive">+$${product.simulated.earnings.toFixed(2)}</span>
                    </div>
                    <div class="stat-row real">
                        <span class="stat-label">💎 Reel:</span>
                        <span class="stat-amount">$${this.formatAmount(product.real.amount)}</span>
                        <span class="stat-earnings positive">+$${product.real.earnings.toFixed(2)}</span>
                    </div>
                    <div class="stat-row total-row">
                        <span class="stat-label">Total:</span>
                        <span class="stat-amount total-amount">$${this.formatAmount(product.simulated.amount + product.real.amount)}</span>
                        <span class="stat-earnings total-earnings">+$${(product.simulated.earnings + product.real.earnings).toFixed(2)}</span>
                    </div>
                </div>
            </div>
        `).join('');
        ...
        return `
            <div class="investment-item ${typeClass}">
                ...
                <div class="inv-values">
                    ...
                    <span class="inv-earnings positive">+${inv.earnings.toFixed(2)} (${pnlPct}%)</span>
    ...
    async addRealFromInput() {
        ...
        if (!isConnected) {
            this.showNotification('Connectez votre wallet pour deposer des fonds reels', 'warning');
        ...
        if (confirmed) {
            ...
            if (result.success) {
                this.showNotification(`+$${amount.toLocaleString()} USDC depose depuis votre wallet!`, 'success');
                this.openModal(); // Refresh modal
            } else {
                this.showNotification(result.error, 'error');
            }
    ...
    /**
     * Verifier si un wallet est connecte
     */
    checkWalletConnected() {
        // Verifier les differentes sources de connexion wallet
        if (typeof window.walletAddress !== 'undefined' && window.walletAddress) {
            return true;
        }
        if (typeof WalletConnect !== 'undefined' && WalletConnect.address) {
    ...
    showWalletConnectDialog() {
        ...
        dialog.innerHTML = `
            <div class="wallet-dialog-content">
                <h3>🔗 Connecter Wallet</h3>
                <p>Connectez votre wallet pour deposer des fonds reels</p>
            ...
            dialog.innerHTML = `
                <div class="deposit-dialog-content">
                    ...
                    <div class="deposit-actions">
                        <button class="btn-confirm" onclick="document.getElementById('deposit-confirm-dialog').remove(); window._depositResolve(true);">
                            ✅ Confirmer le Depot
                        </button>
                        <button class="btn-cancel" onclick="document.getElementById('deposit-confirm-dialog').remove(); window._depositResolve(false);">
                            Annuler
                        </button>
                    </div>
                </div>
    ...
    updateMainPortfolio() {
        ...
        if (pnlEl) pnlEl.textContent = (totals.earnings >= 0 ? '+' : '') + '$' + totals.earnings.toFixed(2);
    ...
    updateUI() {
        ...
        if (modal) {
            ...
            if (earningsEl) earningsEl.textContent = '+$' + totals.earnings.toFixed(2);
            if (totalEl) totalEl.textContent = '$' + this.formatAmount(totals.total);
