...
const UniswapAPI = {
    // API endpoints - Multiple aggregators for reliability
    ...
    API_KEYS: {
        '0x': '', // Get from: https://0x.org/docs/introduction/getting-started
        '1inch': '' // Get from: https://portal.1inch.dev/
    },

    // Preferred aggregator order
    AGGREGATOR_PRIORITY: ['1inch', '0x'],
    ...
    CHAINS: {
        ...
        FANTOM: { id: 250, name: 'Fantom', rpc: 'https://rpc.ftm.tools' },
        SOLANA: { id: 0, name: 'Solana', rpc: 'https://api.mainnet-beta.solana.com' },
        BSC: { id: 56, name: 'BNB Chain', rpc: 'https://bsc-dataseed.binance.org' }
    },
    ...
    TOKENS: {
        ...
        JUP: {
            ...
            decimals: 6,
            addresses: {
                0: 'JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN'
            }
        },
        BNB: {
            symbol: 'BNB',
    ...
    // === SWAP SECURITY ===
    swapSecurity: {
        maxSlippageAllowed: 5,        // 5% max slippage
        maxPriceImpactAllowed: 3,     // 3% max price impact
        maxSingleSwapUSD: 50000,      // $50k max single swap
        rateLimiter: { count: 0, windowStart: 0, maxPerMinute: 10 },
        pausedChains: {},             // { chainId: reason }
        recentSwaps: [],              // Track for anomaly detection
        suspiciousTokens: new Set(),  // Flagged token addresses
    },

    /**
     * Pre-swap security checks
     */
    validateSwap(fromToken, toToken, amount, quote) {
        const sec = this.swapSecurity;

        // Rate limiting
        const now = Date.now();
        if (now - sec.rateLimiter.windowStart > 60000) {
            sec.rateLimiter.count = 0;
            sec.rateLimiter.windowStart = now;
        }
        if (sec.rateLimiter.count >= sec.rateLimiter.maxPerMinute) {
            return { valid: false, reason: 'Rate limit: too many swaps per minute' };
        }

        // Chain pause check
        if (sec.pausedChains[this.currentChain]) {
            return { valid: false, reason: `Chain paused: ${sec.pausedChains[this.currentChain]}` };
        }

        // Cross-check with ArbitrageScanner if available
        if (typeof ArbitrageScanner !== 'undefined') {
            const chainMap = { 1: 'ethereum', 42161: 'arbitrum', 137: 'polygon', 8453: 'base', 10: 'optimism', 250: 'fantom', 56: 'bsc' };
            const networkId = chainMap[this.currentChain];
            if (networkId) {
                const chainSafe = ArbitrageScanner.isChainSafe(networkId);
                if (!chainSafe.safe) return { valid: false, reason: `Chain security alert: ${chainSafe.reason}` };
            }
        }

        // Suspicious token check
        const fromAddr = this.getTokenAddress(fromToken);
        const toAddr = this.getTokenAddress(toToken);
        if (fromAddr && sec.suspiciousTokens.has(fromAddr.toLowerCase())) {
            return { valid: false, reason: `Token ${fromToken} flagged as suspicious` };
        }
        if (toAddr && sec.suspiciousTokens.has(toAddr.toLowerCase())) {
            return { valid: false, reason: `Token ${toToken} flagged as suspicious` };
        }

        // Quote validation
        if (quote) {
            if (quote.priceImpact > sec.maxPriceImpactAllowed) {
                return { valid: false, reason: `Price impact too high: ${quote.priceImpact.toFixed(2)}% (max ${sec.maxPriceImpactAllowed}%)` };
            }
        }
     ...
     */
    flagToken(address, reason) {
        this.swapSecurity.suspiciousTokens.add(address.toLowerCase());
        console.error(`[SWAP SECURITY] Token flagged: ${address} - ${reason}`);
    },

    /**
     * Pause swaps on a chain
     */
    pauseSwapChain(chainId, reason) {
        this.swapSecurity.pausedChains[chainId] = reason;
        console.error(`[SWAP SECURITY] Chain ${chainId} paused: ${reason}`);
    },

    resumeSwapChain(chainId) {
        delete this.swapSecurity.pausedChains[chainId];
    },
    ...
    /**
     * Execute swap transaction with platform fee
     */
    async executeSwap(quote, wallet, privateKey) {
        ...
        const validation = this.validateSwap(quote.fromToken, quote.toToken, quote.fromAmount, quote);
        if (!validation.valid) {
            throw new Error(`[SECURITY BLOCK] ${validation.reason}`);
        }

        // Increment rate limiter
        this.swapSecurity.rateLimiter.count++;

        // Track swap for anomaly detection
        this.swapSecurity.recentSwaps.push({
            from: quote.fromToken, to: quote.toToken,
            amount: quote.fromAmount, timestamp: Date.now()
        });
        // Keep only last 50
        if (this.swapSecurity.recentSwaps.length > 50) {
            this.swapSecurity.recentSwaps = this.swapSecurity.recentSwaps.slice(-50);
        }

        // Calculate platform fee (0.1%)
        const swapValue = parseFloat(quote.amountUSD || quote.amountIn || 0);
        const fee = window.FeeConfig ? window.FeeConfig.calculateFee(swapValue) : { feeAmount: 0 };

        console.log(`[Swap] Amount: $${swapValue}, Platform fee: $${fee.feeAmount.toFixed(4)}`);
        ...
        if (wallet.type === 'metamask' && window.ethereum) {
            try {
                ...
                // Log fee collection (fee is built into quote from aggregator)
                if (window.FeeConfig && fee.feeAmount > 0) {
                    window.FeeConfig.logFeeCollection(txHash, fee.feeAmount, 'ethereum');
                }
