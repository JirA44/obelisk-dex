...
const LeveragedTokensModule = {
    ...
    render(containerId) {
        ...
        if (!el) return;
        el.innerHTML = '<h3 style="color:#00ff88;margin-bottom:16px;">Leveraged Tokens</h3><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;">' + this.tokens.map(t => 
            '<div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:16px;"><strong>' + t.name + '</strong><br>' + t.leverage + 'x ' + t.direction.toUpperCase() + '<br>Fee: ' + t.fee + '%<br><button onclick="LeveragedTokensModule.quickBuy(\'' + t.id + '\')" style="margin-top:10px;padding:8px 16px;background:#00ff88;border:none;border-radius:8px;color:#000;font-weight:bold;cursor:pointer;">Buy</button></div>'
        ).join('') + '</div>';
    },
    quickBuy(tokenId) {
