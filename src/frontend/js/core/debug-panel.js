...
const DebugPanel = {
    ...
    init() {
        ...
        // Admin mode: ?debug=admin or localStorage admin flag
        const urlParams = new URLSearchParams(window.location.search);
        const debugParam = urlParams.get('debug');
        const isLocalAdmin = localStorage.getItem('obelisk_admin') === 'true';

        if (debugParam !== 'admin' && !isLocalAdmin) {
            return; // Not admin - don't show debug panel
        }

        this.isAdmin = true;
        // Show footer debug button for admins
        const footerDebugBtn = document.getElementById('btn-footer-debug');
        if (footerDebugBtn) footerDebugBtn.style.display = '';
        this.createPanel();
        this.interceptConsole();
        this.interceptErrors();
        this.log('info', 'Debug Panel initialized (Admin Mode)');
    },
