...
const RealTrading = {
    ...
    hashAction(action, timestamp) {
        const payload = JSON.stringify(action) + timestamp;
        // Use keccak256 from ethers.js for Hyperliquid-compatible hashing
        if (typeof ethers !== 'undefined' && ethers.keccak256 && ethers.toUtf8Bytes) {
            return ethers.keccak256(ethers.toUtf8Bytes(payload));
        }
        // Fallback: ethers not loaded yet
        throw new Error('ethers.js required for order signing (keccak256)');
    },
