...
const DemoTrading = {
    ...
    async placeOrder(pair, side, size, leverage = 5) {
        ...
        // Try PriceService first (offline mode)
        if (typeof PriceService !== 'undefined' && PriceService.getPrice) {
            price = PriceService.getPrice(coin);
        }

        // Try RealtimePrices as fallback
        if (!price && typeof RealtimePrices !== 'undefined' && RealtimePrices.getPrice) {
    ...
    async closePosition(positionId) {
        ...
        // Get current price from PriceService (offline mode)
        let currentPrice;
        if (typeof PriceService !== 'undefined' && PriceService.getPrice) {
            currentPrice = PriceService.getPrice(position.coin);
        } else if (typeof RealtimePrices !== 'undefined') {
    ...
    startPositionUpdater() {
        setInterval(async () => {
            ...
            for (const pos of this.account.positions) {
                try {
                    // Get live price from PriceService (offline mode)
                    let price;

                    // PriceService first (offline)
                    if (typeof PriceService !== 'undefined' && PriceService.getPrice) {
                        price = PriceService.getPrice(pos.coin);
                    }

                    // RealtimePrices fallback
                    if (!price && typeof RealtimePrices !== 'undefined' && RealtimePrices.getPrice) {
