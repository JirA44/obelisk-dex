/**
 * BASE EXECUTOR - Coinbase L2
 * Executes trades on Base (Coinbase Layer 2)
 *
 * Version: 1.0
 * Date: 2026-02-17
 */
...
class BaseExecutor {
    constructor(config = {}) {
        ...
        const envKey = this.network === 'MAINNET'
            ? (process.env.BASE_MAINNET_PRIVATE_KEY || process.env.ARBITRUM_PRIVATE_KEY)
            : (process.env.BASE_TESTNET_PRIVATE_KEY || process.env.ARBITRUM_PRIVATE_KEY);

        if (envKey) {
            try {
                this.wallet = new ethers.Wallet(envKey, this.provider);
                console.log(`✅ Base Executor initialized (${this.config.name})`);
                console.log(`   Wallet: ${this.wallet.address}`);
            } catch (error) {
                console.warn('⚠️  Failed to load Base wallet:', error.message);
                this.wallet = null;
            }
        } else {
            this.wallet = null;
