...
const RealtimePrices = {
    ...
    /**
     * Fetch prices from real DEX API with timing
     */
    async fetchPrices() {
        const metrics = this.state.metrics;
        ...
        try {
            ...
            metrics.apiLatencyAvg = metrics.apiLatencyAvg === 0
                ? latency
                : (metrics.apiLatencyAvg * 0.9 + latency * 0.1);

            // Estimate network RTT (roughly half of total latency for simple requests)
            metrics.networkRtt = latency * 0.6;
            ...
            if (data.markets) {
                ...
                // Estimate download speed (bytes / time)
                const responseSize = JSON.stringify(data).length;
                metrics.downloadSpeed = (responseSize / latency) * 1000; // bytes per second

                data.markets.forEach(market => {
                    const coin = market.pair.replace('/USDC', '');
                    const oldPrice = this.state.prices[coin]?.price || market.price;
                    ...
                    this.state.prices[coin] = {
                        ...
                        timestamp: now,
                        direction: market.price > oldPrice ? 'up' : market.price < oldPrice ? 'down' : 'neutral'
                    };
    ...
    async fetchArbitrage() {
        try {
            const response = await fetch(`${this.config.apiUrl}/api/arbitrage`);
            const data = await response.json();

            if (data.opportunities) {
                this.state.arbitrage = data.opportunities;
        ...
        panel.innerHTML = `
            ...
            ">
                ...
                " onclick="toggleObeliskLive()">
                    ...
                    <button id="obelisk-live-toggle" style="
    ...
    createChart(containerId, coin, options = {}) {
        ...
        const updateChart = () => {
            ...
            const history = this.getHistory(coin, 100);
            if (history.length < 2) {
